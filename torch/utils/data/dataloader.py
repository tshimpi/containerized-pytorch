...
class DataLoader(Generic[T_co]):
    ...
    def _get_iterator(self) -> '_BaseDataLoaderIter':
        ...
        elif self.containerize == True:
            self.check_worker_number_rationality()
            return _MultiProcessingRPCDataLoaderIter(self)
        else:
            self.check_worker_number_rationality()
...
class _MultiProcessingRPCDataLoaderIter(_BaseDataLoaderIter):
    ...
    def __init__(self, loader):
        ...
        print("Called pytorch RPC dataloader")
        ...
        # NOTE: Have removed the parts corresponding to the pin-memory thread.
    ...
    def _reset(self, loader, first_iter=False):
        ...
        self._worker_queue_idx_cycle = itertools.cycle(range(self._num_workers))
        # We resume the prefetching in case it was enabled
        print("Called reset in dataloader")
        if not first_iter:
            for idx in range(self._num_workers):
                self.put_index_rpc(idx, _utils.worker._ResumeIteration(self._shared_seed))
            resume_iteration_cnt = self._num_workers
            while resume_iteration_cnt > 0:
                return_idx, return_data = self._get_data()
                if isinstance(return_idx, _utils.worker._ResumeIteration):
    ...
    def _try_get_data(self, timeout=_utils.MP_STATUS_CHECK_INTERVAL):
        ...
        #   (bool: whether successfully get data, any: data if successful else None)
        try:
            data = self.local_data_queue.get(timeout=timeout)
            return (True, data)
        except Exception as e:
            # At timeout and error, we manually check whether any worker has
            # failed. Note that this is the only mechanism for Windows to detect
...
# (shell2) ./test_socket.py sock_tmp 1017 send
    ...
    # Callback adds the worker result to the local data queue
    def worker_callback(self, future):
        print("Callback called")
        worker_result = future.value()
        if worker_result is  None:
            return
        self.local_data_queue.put(worker_result)
        print(worker_result)
        print("Put the result in the local data queue")
    ...
    def _try_put_index(self):
        ...
        self.put_index_rpc(worker_queue_idx, index)
        self._task_info[self._send_idx] = (worker_queue_idx,)
        self._tasks_outstanding += 1
        self._send_idx += 1
    ...
    def _mark_worker_as_unavailable(self, worker_id, shutdown=False):
        ...
        # process.
        # q.put(None)
        self.put_index_rpc(worker_id, None)
